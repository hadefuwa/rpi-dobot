...
class PLCClient:
    ...
    def read_target_pose(self, db_number: int = 1) -> Dict[str, float]:
        """Read target X, Y, Z position from PLC (offset 0, 4, 8)"""
        return {
            'x': self.read_db_real(db_number, 0) or 0.0,
            'y': self.read_db_real(db_number, 4) or 0.0,
            'z': self.read_db_real(db_number, 8) or 0.0
        }

    def write_current_pose(self, pose: Dict[str, float], db_number: int = 1):
        """Write current X, Y, Z position to PLC (offset 12, 16, 20)"""
        self.write_db_real(db_number, 12, pose.get('x', 0.0))
        self.write_db_real(db_number, 16, pose.get('y', 0.0))
