...
def vision_detect():
    """Run defect detection on current frame (optionally after object detection)"""
    if camera_service is None:
        return jsonify({'error': 'Camera service not initialized'}), 503
    ...
    try:
        ...
        if roi_regions:
            # Run defect detection on each ROI
            all_defects = []
        ...
        # Add detected objects to results
        ...
        # Optionally draw defects and objects on frame
...
def vision_analyze():
    ...
    try:
        ...
        # Draw objects and defects on frame
        annotated_frame = frame.copy()
        if detected_objects:
            annotated_frame = camera_service.draw_objects(annotated_frame, detected_objects, color=(0, 255, 0))
        if results.get('defects_found'):
